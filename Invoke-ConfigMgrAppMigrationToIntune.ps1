...
function Write-BrandHeader
{
    ...
    Write-Host '  This script is provided as-is and should be validated in a test environment' -ForegroundColor DarkYellow
    Write-Host '  before production use. Review all outputs, packaging results, and Intune' -ForegroundColor DarkYellow
    Write-Host '  objects created by the script.' -ForegroundColor DarkYellow
    Write-Host ''
}
...
function Show-MainMenu
{
    ...
    Write-Host '  [1] Document applications from ConfigMgr' -ForegroundColor Cyan
    Write-Host '  [2] Create IntuneWin packages' -ForegroundColor Cyan
    Write-Host '  [3] Upload applications to Intune' -ForegroundColor Cyan
    ...
    do
    {
        ...
        switch -Regex ($choice)
        {
            '^1$'      { return 'GetConfigMgrAppInfo' }
            '^2$'      { return 'CreateIntuneWinFiles' }
            '^3$'      { return 'UploadAppsToIntune' }
            '^(Q|q)$'  { return 'Quit' }
            default
            {
...
function Initialize-InteractiveParameters
{
    [CmdletBinding()]
    param()
    ...
    {
        $script:ExportFolder = Read-RequiredValue -Prompt 'Export folder' -DefaultValue 'C:\ExportToIntune'
    }

    if ($GetConfigMgrAppInfo -or $RunAllActions)
    {
        if ([string]::IsNullOrWhiteSpace($Sitecode))
        {
            $script:Sitecode = Read-RequiredValue -Prompt 'ConfigMgr SiteCode'
        }

        if ([string]::IsNullOrWhiteSpace($Providermachinename))
        {
            $script:Providermachinename = Read-RequiredValue -Prompt 'ConfigMgr ProviderMachineName'
        }
    }

    if (($UploadAppsToIntune -or $RunAllActions) -and (-not [string]::IsNullOrWhiteSpace($EntraIDAppID)) -and [string]::IsNullOrWhiteSpace($EntraIDTenantID))
    {
...
function Write-CMTraceLog 
{
    ...
    switch ($Severity)
    {
        ...
        "Warning"     { $cmSeverity=2; $color = [System.ConsoleColor]::Yellow; $tag = 'WARN '; break }
        "Error"       { $cmSeverity=3; $color = [System.ConsoleColor]::Red;    $tag = 'ERROR'; break }
    }

    $timeStamp = Get-Date -Format 'HH:mm:ss'
    $trimmedMessage = if ($null -eq $Message) { '' } else { $Message.TrimEnd() }

    if ([string]::IsNullOrWhiteSpace($trimmedMessage))
    {
        $consoleMessage = ''
    }
    elseif ($trimmedMessage -match '^(Start of|End of|Runtime:|Scriptversion:|Export will be made to folder:|Will get list of|Start creating|Install required modules|Open Out-GridView|Total apps to|Waiting for Intune service|Creating Win32 app|Request content version|Sending content metadata|Committing the file|Setting the commited content version)')
    {
        $consoleMessage = "`n[$timeStamp] === $trimmedMessage ==="
        $color = [System.ConsoleColor]::White
    }
    else
    {
        $consoleMessage = "[$timeStamp] [$tag] $trimmedMessage"
    }
...
#region validate required parameters after menu/input
if ([string]::IsNullOrWhiteSpace($ExportFolder))
{
    Write-Host "ExportFolder is required." -ForegroundColor Red
    Exit 1
}

if (($GetConfigMgrAppInfo -or $RunAllActions) -and [string]::IsNullOrWhiteSpace($Sitecode))
{
    Write-Host "SiteCode is required for the selected action." -ForegroundColor Red
    Exit 1
}

if (($GetConfigMgrAppInfo -or $RunAllActions) -and [string]::IsNullOrWhiteSpace($Providermachinename))
{
    Write-Host "ProviderMachineName is required for the selected action." -ForegroundColor Red
    Exit 1
}
#endregion
...
try 
{
    ...
    {
        Write-ConsoleSection -Title "Preparing export structure"
        Write-ConsoleDetail -Message "Export folder not found. Creating: $ExportFolder"
    ...
    foreach ($folder in ($ExportFolderTools, $ExportFolderAppDetails, $ExportFolderIcons, $ExportFolderScripts, $ExportFolderWin32Apps))
    {
        ...
        {
            Write-ConsoleDetail -Message "Creating folder: $folder"
            New-Item -ItemType Directory -Path $folder -Force | Out-Null   
        }
        else 
        {
            Write-ConsoleDetail -Message "Folder exists: $folder"
        }
    }
}
catch 
...
if ($GetConfigMgrAppInfo -or $RunAllActions)
{
    ...
    if ($selectedApps)
    {
        ...
        {
            $appCounter++
            Write-Progress -Activity "Analyzing ConfigMgr applications" -Status ("App {0}/{1} - {2}" -f $appCounter, $selectedApps.count, $app.LocalizedDisplayName) -PercentComplete ($appCounter / $selectedApps.count*100)
    ...
    Write-Progress -Completed -Activity "Analyzing ConfigMgr applications"
...
if ($CreateIntuneWinFiles -or $CreateIntuneWinFilesAndUploadToIntune -or $RunAllActions)
{
    ...
    {
        Write-CMTraceLog -Message "File not found: `"$($ExportFolder)\AppDetails\AllAps.xml`". Run the script with the GetConfigMgrAppInfo switch" -Severity Error
    ...
    if ($appInObj.count -eq 0)
    {
        ...
        Write-CMTraceLog -Message "Re-run the script with parameter: `"-GetConfigMgrAppInfo`" first." -Severity Warning
        Write-CMTraceLog -Message "End of script"
        break
    }
    ...
    foreach($configMgrApp in $appInObj)
    {
        ...
        $appCounter++
        Write-Progress -Activity "Creating .intunewin packages" -Status ("App {0}/{1} - {2}" -f $appCounter, $selectedAppList.count, $configMgrApp.Name) -PercentComplete ($appCounter / $selectedAppList.count*100)
    ...
    Write-Progress -Completed -Activity "Creating .intunewin packages"
...
if ($UploadAppsToIntune -or $CreateIntuneWinFilesAndUploadToIntune -or $RunAllActions)
{
    ...
    foreach($configMgrApp in $appInObj)
    {
        ...
        $appCounter++
        Write-Progress -Id 0 -Activity "Uploading applications to Intune" -Status ("App {0}/{1} - {2}" -f $appCounter, $selectedAppList.count, $configMgrApp.Name) -PercentComplete ($appCounter / $selectedAppList.count*100)
        if ($configMgrApp.AppImportToIntunePossible -ine 'Yes')
        {
        ...
        try 
        {
            for ($Chunk = 0; $Chunk -lt $ChunkCount; $Chunk++) 
            {
                ...
                Write-Progress -Id 1 -ParentId 0 -Activity "Uploading content to Azure Storage" -Status ("Chunk {0}/{1}" -f $CurrentChunk, $ChunkCount) -PercentComplete ($CurrentChunk / $ChunkCount*100)
        ...
        Write-Progress -Id 1 -ParentId 0 -Completed -Activity "Uploading content to Azure Storage"
